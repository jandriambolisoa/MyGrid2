...
from backend.src.events.dependencies import valid_championship_id, get_upcoming_event, \
    is_session_over, get_event_championship, valid_event_id, get_event_from_id
...
@router.get("/home/events/{event_id}", response_model=NavMainEvent, status_code=status.HTTP_200_OK)
async def home_get_one_event(event_id: int = Depends(valid_event_id), language: str = Depends(get_current_user_language), db: Database = Depends(get_db), current_user: UserSelf = Depends(get_current_user)):
    event = await get_event_from_id(event_id, language)
    championship = await get_event_championship(event.id, language)
