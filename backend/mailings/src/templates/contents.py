...
from backend.mailings.src.templates.constants import VALID_HTML_DECL
from backend.mailings.src.templates import exceptions as templates_exceptions

def get_contents_folder():
    folderpath = os.path.join(os.path.dirname(os.path.abspath(__file__)), "contents")
...
class HTMLContentValidator(HTMLParser):
    ...
    def feed(self, data):
        ...
        if (not self.has_html_decl
            ...
            raise templates_exceptions.InvalidHTMLException(
