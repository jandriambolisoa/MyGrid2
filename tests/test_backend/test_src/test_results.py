...
def test_get_session_results(test_upcoming_events, test_passed_events, test_results, unauthorized_user, unverified_user, authorized_user, moderator_user, banned_user):
    ...
    assert mock_get_session_results(unauthorized_user, upcoming_session) == status.HTTP_401_UNAUTHORIZED
    assert mock_get_session_results(unverified_user, upcoming_session) == status.HTTP_404_NOT_FOUND
    ...
    assert mock_get_session_results(unauthorized_user, passed_session) == status.HTTP_401_UNAUTHORIZED
    assert mock_get_session_results(unverified_user, passed_session) == status.HTTP_200_OK
...
def test_override_session_results(test_upcoming_events, test_drivers, test_passed_events, test_results, unauthorized_user, unverified_user, authorized_user, moderator_user, banned_user):
    upcoming_event = random.choice(test_upcoming_events)
    upcoming_session = get_competitive_sessions_from_event_id(upcoming_event.id)[0]
    assert mock_override_session_results(unauthorized_user, upcoming_session, test_drivers) == status.HTTP_401_UNAUTHORIZED
    assert mock_override_session_results(unverified_user, upcoming_session, test_drivers) == status.HTTP_403_FORBIDDEN
    ...
    passed_event = random.choice(test_passed_events)
    passed_session = get_competitive_sessions_from_event_id(passed_event.id)[0]
    assert mock_override_session_results(unauthorized_user, passed_session, test_drivers) == status.HTTP_401_UNAUTHORIZED
    assert mock_override_session_results(unverified_user, passed_session, test_drivers) == status.HTTP_403_FORBIDDEN
...
def test_remove_session_results(test_upcoming_events, test_drivers, test_passed_events, test_results, unauthorized_user, unverified_user, authorized_user, moderator_user, banned_user):
    upcoming_event = random.choice(test_upcoming_events)
    upcoming_session = get_competitive_sessions_from_event_id(upcoming_event.id)[0]
    assert mock_remove_session_results(unauthorized_user, upcoming_session) == status.HTTP_401_UNAUTHORIZED
    assert mock_remove_session_results(unverified_user, upcoming_session) == status.HTTP_403_FORBIDDEN
    ...
    passed_event = random.choice(test_passed_events)
    passed_session = get_competitive_sessions_from_event_id(passed_event.id)[0]
    assert mock_remove_session_results(unauthorized_user, passed_session) == status.HTTP_401_UNAUTHORIZED
    assert mock_remove_session_results(unverified_user, passed_session) == status.HTTP_403_FORBIDDEN
