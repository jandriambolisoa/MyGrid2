...
def mock_home_get_championships(user_obj: MockUser, championship):
    res = user_obj.client.get(f"/nav/home/championships", params={"championship_id": championship.id})
    return res.status_code

def test_home_get_championships(test_championship, test_results, unauthorized_user, unverified_user, authorized_user, moderator_user,
                             banned_user):
    assert mock_home_get_championships(unauthorized_user, test_championship) == status.HTTP_401_UNAUTHORIZED
    assert mock_home_get_championships(unverified_user, test_championship) == status.HTTP_200_OK
    assert mock_home_get_championships(authorized_user, test_championship) == status.HTTP_200_OK
    assert mock_home_get_championships(moderator_user, test_championship) == status.HTTP_200_OK
    assert mock_home_get_championships(banned_user, test_championship) == status.HTTP_401_UNAUTHORIZED
